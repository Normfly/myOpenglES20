...
void main()
{
	    ...
	    vec4 bumpColor = texture2D(u_texture, v_bumpCoord);// get bump map color, just use green channel for brightness, for now
        ...
        gl_FragColor = (v_Color * diffuse * texColor * bumpColor.g);// add ambient and directional lighting (modifies alpha channel as well, fix in next line)
